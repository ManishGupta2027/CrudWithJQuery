jQuery.fn.daterangepicker = function (settings) {
    ...
    if (rangeInput.size() == 2) {
        ...
        var teampData = rangeInput.val().split(options.rangeSplitter)[0];
        if (teampData != null && teampData != undefined) {
            var dateArray = teampData.split('/');
            var day = $.trim(dateArray[0]);
            var month = $.trim(dateArray[1]);
            var year = $.trim(dateArray[2]);
            inputDateAtemp = new Date(year + "-" + month + "-" + day);
        }
        else {
